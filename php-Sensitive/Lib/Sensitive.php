<?php

/**
 * 敏感词类
 * User: gaojun<godwin.gao@huolala.cn>
 * Date: 2018/11/13
 * Time: 下午3:49
 */

namespace Lib;

require_once dirname(__DIR__) . '/Lib/RedisStore.php';

class Sensitive
{
    /**
     * 替换码
     * @var string
     */
    private $replaceCode = '*';
    /**
     * 敏感词库集合
     * @var array
     */
    private $trieTreeMap = array();

    /**
     * 敏感词计数
     * @var int
     */
    private $count = 0;

    /**
     * 敏感词每次读取限制
     * @var int
     */
    private $limit = 5000;

    protected $key = 's:w:l';

    /**
     * @var RedisStore
     */
    protected $redis;

    /**
     * 干扰因子集合
     * @var array
     */
    private $disturbList = array("*", " ", "&");

    public function __construct($disturbList = array())
    {
        $this->redis = RedisStore::getInstance();
        $this->disturbList = empty($disturbList) ? $this->disturbList : $disturbList;
    }

    /**
     * 添加敏感词
     * @param array $wordsList
     * @return bool
     * @throws \Exception
     */
    public function addWords(array $wordsList)
    {

        foreach ($wordsList as $words) {
            $map = [];
            $nowWords = &$map;
            $len = mb_strlen($words);
            for ($i = 0; $i < $len; $i++) {
                $word = mb_substr($words, $i, 1);
                if (!isset($nowWords[$word])) {
                    $nowWords[$word] = false;
                }
                $nowWords = &$nowWords[$word];
            }
            try {
                $res = $this->redis->add($this->key, $map);
                if (!$res) {
                    continue;
                }
            } catch (\Exception $e) {
                throw new \Exception($e->getMessage());
            }

        }

        return true;
    }

    /**
     * 查找对应敏感词
     * @param $txt
     * @param bool $hasReplace
     * @param array $replaceCodeList
     * @return array
     */
    public function search($txt, $hasReplace = false, &$replaceCodeList = array())
    {

        $wordsList = array();
        $offset = 0;
        while (true) {
            $this->trieTreeMap = $this->redis->getList($this->key, $offset, $this->limit - 1);
            if(empty($this->trieTreeMap)) {
                break;
            }
            $txtLength = mb_strlen($txt);
            for ($i = 0; $i < $txtLength; $i++) {
                $wordLength = $this->checkWord($txt, $i, $txtLength);
                if ($wordLength > 0) {
                    $words = mb_substr($txt, $i, $wordLength);
                    $wordsList[] = $words;
                    $hasReplace && $replaceCodeList[] = str_repeat($this->replaceCode, mb_strlen($words));
                    $i += $wordLength - 1;
                }
            }
            $offset += $this->limit;
        }

        return $wordsList;
    }

    public function getTotal()
    {
        return $this->redis->getCount($this->key);
    }

    /**
     * 过滤敏感词
     * @param $txt
     * @return mixed
     */
    public function filter($txt)
    {
        $replaceCodeList = array();
        $wordsList = $this->search($txt, true, $replaceCodeList);
        if (empty($wordsList)) {
            return $txt;
        }
        return str_replace($wordsList, $replaceCodeList, $txt);
    }

    /**
     * 敏感词检测
     * @param $txt
     * @param $beginIndex
     * @param $length
     * @return int
     */
    private function checkWord($txt, $beginIndex = 0, $length = 0)
    {
        $flag = false;
        $wordLength = 0;
        $trieTree = &$this->trieTreeMap;
        if (empty($trieTree)) {
            return false;
        }
        $length = empty($length) ? mb_strlen($txt) : $length;
        for ($i = $beginIndex; $i < $length; $i++) {
            $word = mb_substr($txt, $i, 1);
            if ($this->checkDisturb($word)) {
                $wordLength++;
                continue;
            }
            if (!isset($trieTree[$word])) {
                break;
            }
            $wordLength++;
            if ($trieTree[$word] !== false) {
                $trieTree = &$trieTree[$word];
            } else {
                $flag = true;
            }
        }
        $flag || $wordLength = 0;
        return $wordLength;
    }

    /**
     * 干扰因子检测
     * @param $word
     * @return bool
     */
    private function checkDisturb($word)
    {
        return in_array($word, $this->disturbList);
    }
}