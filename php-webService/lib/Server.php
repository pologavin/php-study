...
class Server
{
    ...
    private $serviceNamePrefix = '';

    private $wsdlCacheEnabled = 0; // WSDL 缓存：1开启，0关闭

    private $xmlPath = ROOT . '/xml/';
    ...
    public function getWSDL()
    {
        ...
        if (is_file($this->xmlPath . $class->getShortName() . '.wsdl')) {
            return file_get_contents($this->xmlPath . $class->getShortName() . '.wsdl');
        } else {
            /**
            ...
            file_put_contents($this->xmlPath . $class->getShortName() . '.wsdl', $wsdl);
            return $wsdl;
        }
    }
